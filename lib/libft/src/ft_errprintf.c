...
static int	check_sign(char c, va_list args)
{
	...
	len = 0;
	if (c == 'c')
		len += ft_putchar_fd(va_arg(args, int), 2);
	else if (c == 's')
		len = ft_putstr_fd(va_arg(args, char *), 2);
	else if (c == 'i' || c == 'd')
		len = ft_putnbr_fd(va_arg(args, int), 2, 0);
	else if (c == 'u')
		len += ft_putunbr_fd(va_arg(args, unsigned int), 2, 0);
	else if (c == 'x')
...
int	ft_errprintf(const char *str, ...)
{
	va_list	args;
	int		i;
	int		len;
